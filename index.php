...
if (isset($_GET['views'])) { // si existe la variable views en la url se ejecuta el controlador
    $url = explode("/", $_GET['views']); // separamos la url por / y lo guardamos en un array
} else {
    $url = ["login"]; // si no existe la variable views en la url se ejecuta el controlador login
}
...
<body>
    ...
    <?php
    // el auto load se encarga de incluir las clases que se necesiten como esa clase viewsController
    use app\controllers\viewsController;

    $viewsController = new viewsController(); // se instancia la clase viewsController para poder usar el metodo getView
    $vista = $viewsController->obtenerVistasControlador($url[0]); // se obtiene la vista que se va a mostrar 
    if ($vista == "login" || $vista == "404") { // si la vista es login o 404 se incluye el navbar
                            ...
                            <li class="relative">
                                ...
                                <template x-if="isProfileMenuOpen">
                                    <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click.away="closeProfileMenu" @keydown.escape="closeProfileMenu" class="absolute right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-gray-100 rounded-md shadow-md dark:border-gray-700 dark:text-gray-300 dark:bg-gray-700" aria-label="submenu">
                                        <li class="flex">
                                            <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200" href="<?php echo APP_URL ?>actualizarUsuario">
                                        ...
                                        <li class="flex">
                                            <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200" href="<?php echo APP_URL ?>ajustes">
                                                <svg class="w-4 h-4 mr-3" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <li class="flex">
                                            <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200" href="<?php echo APP_URL ?>cerraSesion">
                                                <svg class="w-4 h-4 mr-3" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <li class="flex">
                                            <a id="botonPantallaCompleta" class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200" href="#">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(240, 248, 255);transform: 1px ;">
